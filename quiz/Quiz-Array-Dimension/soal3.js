...
function minDistanceBetweenGreatest(arr) {
    // your code here
    let maks = -Infinity;
    for (let i = 0; i < arr.length; i++) {      //tentukan nilai maks
        if (arr[i] > maks) {
            maks = arr[i];
        }
    }
    ...
    let index = [];
    for (let i = 0; i < arr.length; i++) {      //cari indeks masing masing dari nilai maks
        if (arr[i] == maks) {
            index.push(i + 1);
        }
    }
    ...
    if (index.length < 2) {
        ...
        let jarak = [];
        for (let i = 0; i < index.length - 1; i++) {        //mencari jarak antara nilai maks
            jarak.push(Math.abs(index[i] - index[i + 1]));
        }
        jarak.sort();
        return jarak[0];
    }
